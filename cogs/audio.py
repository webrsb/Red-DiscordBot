...
class Audio:
    ...
    async def _create_ffmpeg_player(self, server, filename, local=False):
        ...
        if voice_client is None:
            ...
            if to_connect is None:
                raise VoiceNotConnected("也許我們還沒連線或"
                                        " 我們沒有一個能去的頻道"
                                        " 重新連線. 換句話說就是…LOL"
                                        " REKT.")
    ...
    async def audioset_cachemax(self, size: int):
        ...
        await self.bot.say("最大暫存數為{} MB.".format(size))
        self.save_settings
    ...
    async def audioset_maxlength(self, length: int):
        """每個連結的最大的音樂長度 (秒)"""
        if length <= 0:
            await self.bot.say("啊, 長度不應有負的...你真是不聰明")
            return
        self.settings["MAX_LENGTH"] = length
        await self.bot.say("音樂最大長度為 {} 秒.".format(length))
        self.save_settings()
    ...
    async def audioset_volume(self, ctx, percent: int):
        ...
        if percent >= 0 and percent <= 100:
            ...
            await self.bot.say("聲量現在設置為 " + str(percent) +
                               ". 這個設定會立即生效")
            self.save_settings()
        else:
            await self.bot.say("聲量大小範圍 0 至 100.")
    ...
    async def audioset_vote(self, ctx, percent: int):
        ...
        server = ctx.message.server
        if percent < 0:
            await self.bot.say("不能小於0.")
            return

        if percent > 100:
            percent = 100

        await self.bot.say("投票有效百分比設置為 {}%".format(percent))
    ...
    async def local(self, ctx, name):
        ...
        if not self.voice_connected(server):
            try:
                ...
                return
            if not can_connect:
                await self.bot.say("您所在的頻道不能放音樂，請切換至."
                                   "「小馬音樂台」")
            else:
                await self._join_voice_channel(voice_channel)
        ...
        if self.is_playing(server):
            await self.bot.say("我已經在為這個伺服播放歌曲了!")
    ...
    async def pause(self, ctx):
        ...
        server = ctx.message.server
        if not self.voice_connected(server):
            await self.bot.say("沒有進入這個伺服的語音頻道.")
            return
    ...
    async def play(self, ctx, url):
        ...
        if not self.voice_connected(server):
            try:
                ...
                return
            if not can_connect:
                await self.bot.say("您所在的頻道不能放音樂，"
                                   "請切換至「小馬音樂台」.")
            else:
                await self._join_voice_channel(voice_channel)
        else:  # We are connected but not to the right channel
